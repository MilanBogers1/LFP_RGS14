...
[swr_hpc,swr_pfc,s_hpc,s_pfc,V_hpc,V_pfc,signal2_hpc,signal2_pfc,sd_swr, M_multiplets, Mx_multiplets, multiplets, ripples, Mono_hpc, Mono_pfc]=ripple_detection(HPC,Cortex,states,ss,offset1,offset2,TT,j,i);
...
GC_window_ripples = {};
    for c=1:size(ripples,1)
       waveforms_ripples{c,1}= concatenated_NREM_hpc(int32(ripples(c,5)*1000+1):int32(ripples(c,7)*1000+1));
        if ripples(c,6)*1000+1 < 1200 || ripples(c,6)*1000+1+(1.2*1000) > length(concatenated_NREM_hpc)
          continue     
       else
                   ...
                   if contains(G{i}, 'rial5')
                       clear HPC_1 HPC_2 HPC_3 HPC_4 Cortex_1 Cortex_2 Cortex_3 Cortex_4
                        ...
                        if length(Cortex)<45*60*1000*4
                          ...
                          [swr_hpc, swr_pfc, s_hpc, s_pfc, V_hpc, V_pfc, signal2_hpc, signal2_pfc, sd_swr, M_multiplets, Mx_multiplets, multiplets, ripples, Mono_hpc, Mono_pfc]=ripple_detection(hpc,pfc,states_chunk,ss,offset1,offset2,TT,j,i); 
