...
Wn1=[9/(fn/2) 20/(fn/2)]; % Cutoff=320 Hz
[b2,a2] = butter(3,Wn1); %Filter coefficients
...
for p=1:length(V_pfc)
    ...
    if length(V_pfc_bp2)>4000
        spindles = FindSpindles(V_pfc_bp);
        spindles_total_data{j,1}{i,p} = spindles;
        z=z+size(spindles,1);      
    end
...
total_spindles_minute(j,i)=(total_spindles(j,i)/stage_count*60);
    end
...
total_spindles(j,i+1)=z;
stage_count = sum(states2(:)==ss);
total_spindles_minute(j,i+1)=(total_spindles(j,i+1)/stage_count*60);
    end
clear V_pfc v_pfc V_pfc_bp V_pfc_bp2 v_values vec_bin VV_pfc v2 v_index NC2 NC
...
total_spindles(j,i+2)=z;
stage_count = sum(states3(:)==ss);
total_spindles_minute(j,i+2)=(total_spindles(j,i+2)/stage_count*60);
    end
clear V_pfc v_pfc V_pfc_bp V_pfc_bp2 v_values vec_bin VV_pfc v2 v_index NC2 NC
...
total_spindles(j,i+3)=z;
stage_count = sum(states4(:)==ss);
total_spindles_minute(j,i+3)=(total_spindles(j,i+3)/stage_count*60);
    end
clear V_pfc v_pfc V_pfc_bp V_pfc_bp2 v_values vec_bin VV_pfc v2 v_index NC2 NC
                    end
                    else
