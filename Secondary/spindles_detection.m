function [spindles,phase_total,z,BST,V_pfc,Sx,Ex,Mx] = spindles_detection(vec_bin,NC3,NC2,fn,j,i,k)
...
z = 0;
VV_timestamps_NREM = [0:length(VV_timestamps)-1].'/1000;
...
%% Spindles Detection; Timestamps and Phases
     ...
     if length(VV_timestamps) > 4000 
        spindles = FindSpindlesRGS14(V_pfc_bp,k);
%         spindles_NREM = spindles(:,1:3);
   if ~isempty(spindles)
        spindles_NREM(:,1) = VV_timestamps_NREM(find(ismember(int32(VV_timestamps*1000),int32(spindles(:,1)*1000))));
        spindles_NREM(:,2) = VV_timestamps_NREM(find(ismember(int32(VV_timestamps*1000),int32(spindles(:,2)*1000))));
        spindles_NREM(:,3) = VV_timestamps_NREM(find(ismember(int32(VV_timestamps*1000),int32(spindles(:,3)*1000))));

%         spindles_NREM = FindSpindlesRGS14(V_pfc_bp_NREM,k);
        spindles_total_data{j,1}{i,1} = spindles;
        z = z+size(spindles,1);

        phase_index = find(ismember(int32(VV_timestamps*1000),int32(spindles(:,2)*1000)));
