...
%Convert signal to 1 sec epochs.
  ...
  %% Event Detection 
   [spindles,phase_total,z,BST,V_pfc]= spindles_detection(vec_bin,NC3,NC2,fn,j,i,k);
   if iscell(V_pfc) && ~isempty(spindles)
    concatenated_NREM_pfc = vertcat(V_pfc{:});
    waveforms_spindles={};
        for c=1:size(spindles,1)
           waveforms_spindles{c,1}= concatenated_NREM_pfc(int32(spindles(c,5)*1000+1):int32(spindles(c,7)*1000+1));
        end
    else
...
% %         spindles_total_data = spindles_total_data_complete{j};
        ...
        save(strcat('spindles_waveforms_',g{j},'.mat'),'spindles_waveform_total')
        ...
        save(strcat('spindles_waveforms_compilation_Rat',rat_folder{k},'.mat'),'spindles_waveforms_comp')

    end 
end 
%         cd ..
%         sp_total_data= spindles_total_data{j};
%         sp_uncontinuous_timestamps= spindles_uncontinuous_timestamps{j};
%         sp_phase_total=phase_total{j}.';
% %         total_sp=total_spindles(j);
% %         total_sp_minute=total_spindles_minute(j);
%       spindle_phase_comp= [ spindle_phase_comp;sp_phase_total];
% %       save(strcat('spindles_data',g{j},'.mat'),'sp_total_data','sp_uncontinuous_timestamps','sp_phase_total')
% % clear  sp_total_data  sp_uncontinuous_timestamps phase_total  sp_phase_total
% if j== length(g)
% %  save(strcat('spindles_data_Rat',rat_folder{k},'.mat'),'total_spindles','total_spindles_minute')
% end
%     end
% %     save(strcat('spindle_phase_compilation_Rat',rat_folder{k},'.mat'),'spindle_phase_comp')
% 
%     cd ..
% %     save(strcat('spindles_data_Rat',rat_folder{k},'.mat'),'total_spindles','total_spindles_minute')
% end 
